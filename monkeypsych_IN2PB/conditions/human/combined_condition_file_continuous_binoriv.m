...
SETTINGS.background_image = 'binoriv';

if ~exist('dyn','var') || dyn.trialNumber == 1
    
    
    
    experiments        = {'binoriv main task'};
    ...
    for n_exp = 1:numel(experiments)
        ...
        task.reward.time_neutral            = 0.1; %[0.18 0.18]; % s 
        ...
        switch experiment
            ...
            case 'binoriv main task'
                SETTINGS.GUI_in_acquisition         = 1;
                VAR.binoriv_taskType = 3; % [1 4]
...
%                 photos = 0; % 0: no photos overlayed; 1: photos overlaye
                ...
                % grating-related
                dist_scr = 23; % distance from screen (cm)
...
%                 % from cm to px:
                VAR.width_pxpercm = SETTINGS.screen_w_pix/SETTINGS.screen_w_cm;
...
%                 VAR.wave_length_half = VAR.wave_length/2;
                VAR.x_cent = SETTINGS.screen_w_pix/2;
                VAR.y_cent = SETTINGS.screen_h_pix/2;
                ...
                fix_rad = 0;%1.3; % Radius of detection
                fix_size = 0.2;%0.2; % size of fixation spot; diameter (deg)
...
%% REWARD
VAR.reward_system = false; % if you use rewards
task.reward_lowerBound = 1.5; %sec 
task.reward_upperBound = 2.5;
task.reward_abort = 5; % for binoriv cond.
...
task.timing.fix_time_to_acquire_eye     = 3; % duration of STATE.FIX_ACQ
task.timing.tar_time_to_acquire_eye     = 0; % duration of STATE.FIX_ACQ
task.timing.tar_inv_time_to_acquire_eye = 0;
task.timing.fix_time_hold               = 2.5; % duration of FIX_HOL
...
task.timing.tar_time_to_acquire_eye_bino = 5;%40;%3; % duration of STATE.BIN_RIV_ACQ (allowed time of fixing/switching gaze)
task.timing.tar_time_to_acquire_eye_phys = 5;%40;%3; % duration of STATE.BIN_RIV_ACQ (allowed time of fixing/switching gaze)
task.timing.tar_time_to_acquire_eye = 80; % duration of STATE.BIN_RIV_ACQ in the human task
task.task4_cue_dur = 0.3;
