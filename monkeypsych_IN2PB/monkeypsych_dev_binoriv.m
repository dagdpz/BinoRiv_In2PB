function monkeypsych_dev_binoriv_integratedRefined(subject)
...
if strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation') || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_fixation_continuous') || strcmp(task.custom_conditions, 'D:\monkeypsych\conditions\human\combined_condition_file_continuous_binoriv')...
        || strcmp(task.custom_conditions, 'D:\Sources\MATLAB\monkeypsych_IN2PB\conditions\Bacchus\combined_condition_file_continuous_binoriv')
    global VAR
    
    run(task.custom_conditions);
    if VAR.report
        SETTINGS.dyn.memoryBuffer           = single(nan(SETTINGS.bufferSize,15)); % buffer data before writing to file
    else
    ...
    if VAR.binoriv_taskType == 2 || VAR.binoriv_taskType == 4
        images_binoriv = imread(['BinoRiv_stimuli/without_fp/Face_' num2str(RedImageContrast) '_Object_' num2str(CyanImageContrast) '.tif']);

        images_phys_face = imread(['BinoRiv_stimuli/without_fp/Face_' num2str(RedImageContrast) '_Object_0.tif']);

        images_phys_obj = imread(['BinoRiv_stimuli/without_fp/Face_0_Object_' num2str(CyanImageContrast) '.tif']);
    elseif VAR.binoriv_taskType == 1 || VAR.binoriv_taskType == 3        
    ...
    if SETTINGS.matlab_version(1)>=2012
...
%     set(gca,'Ylim',[-SETTINGS.screen_lh_deg SETTINGS.screen_uh_deg],'Xlim',[-SETTINGS.screen_w_deg/2 SETTINGS.screen_w_deg/2],'XLimMode','manual','YLimMode','manual');
    ...
    if SETTINGS.matlab_version(1)>=2014
        ...
        dyn.eye_position_handle = line(-1000,-1000,'Color','k','Marker','o','XlimInclude','off','YLimInclude','off','EraseMode','xor');%'background');
...
if ~strcmp(monkey_name, 'human') == 1
    ListenChar(2); % Can be a factor to change GUI priority
    ...
    switch dyn.state
        ...
        case STATE.INI_TRI
            ...
            if task.type == 15
                ...
                if VAR.reward_system
                    VAR.reward_time_storage = task.reward_lowerBound + (task.reward_upperBound - task.reward_lowerBound) * rand;
                    VAR.reward = false;
                    VAR.tRewardStock = 0;
                    VAR.tLastReward = 0;
                end
...
%                 aux_draw_GUI_targets(dyn,trial,eff,pha,ang)
        ...
        case STATE.FIX_ACQ  % fixation acquisition
            if task.type == 15
                ...
                stim = [0 0 par_temp.size par_temp.radius]; % (x, y, size)
                stim(3) = deg2pix_withOffset(stim(3),0);
                [stim(1) stim(2)] = deg2pix_xy(stim(1), stim(2));
                par_eye = aux_pr2rect([stim(1)  stim(2)]', stim(3));
                par_eye.pix.x              = stim(1);
                par_eye.pix.y              = stim(2);
                par_eye.pix.size           = stim(3);
                par_eye.stimRect       = aux_pr2rect([stim(1)  stim(2)]', stim(3));
                par_eye.x              = stim(1);
                par_eye.y              = stim(2);
                par_eye.size           = stim(3);
                par_eye.shape          = par_temp.shape;
                par_eye.radiusShape          = 'circle';
                par_eye.stimColor = par_temp.color_dim;
                par_eye.ringColor = [];
                par_eye.arcAngle = 0;
                par_eye.deg.radius = par_temp.radius;
                par_eye.deg.x = 0;
                par_eye.deg.y = 0;
...
%             end
            ...
            if dyn.percept_switch == 0
                VAR.current_trial = VAR.current_trial + 1;
%                 if VAR.num_trial_phys ~= VAR.num_trial_bino
                    if VAR.binoriv_taskType == 4 || VAR.binoriv_taskType == 2
                        if VAR.condition(VAR.current_trial-1) == 1 % fix FPs location during a block
%                             run(task.custom_conditions);
                            task = FP_loc_shuffle(task);
                        end
                    else
%                         run(task.custom_conditions);
                        task = FP_loc_shuffle(task);
...
%                 end
                ...
                if VAR.binoriv_taskType == 1 || VAR.binoriv_taskType == 3
                ...
                elseif VAR.binoriv_taskType == 2 || VAR.binoriv_taskType == 4
                    if VAR.condition(VAR.current_trial) == 2
                        VAR.images = images_phys_face;
                        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
                    elseif VAR.condition(VAR.current_trial) == 3
                        VAR.images = images_phys_obj;
                        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
                    elseif VAR.condition(VAR.current_trial) == 4
                        VAR.images = images_binoriv;
                        binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
                    end
                end
           ...
           if dyn.percept_switch == 0
               if VAR.binoriv_taskType == 4
                   % Sound inducing fixation
                    if dyn.percept_switch == 0    
                        PsychPortAudio('Start', pahandle, 0, 0, 1);
                        [~,dyn,task,~,~] = wait_while_recording_state(task,dyn);
                        PsychPortAudio('Stop', pahandle);
                    end
               end
    ...
    %                     aux_draw_GUI_targets(dyn,trial,'hnd','cue',ang)
           ...
           % pseudo_trial for a run without microstimulation
            ...
            if VAR.fix_trial_duration
                ...
                if VAR.binoriv_taskType == 4
                    dyn.duration = dyn.duration_origin - dyn.time_spent_exploring - task.task4_cue_dur;
                else
...
%                 trial(dyn.trialNumber).task = [trial(dyn.trialNumber).task trial(dyn.trialNumber).task]; 
            ...
            if VAR.reward_system && dyn.time_spent_in_state > task.reward_abort
                VAR.tRewardStock = 0;
                VAR.tLastReward = 0;
            end
        ...
        case STATE.BIN_RIV_HOL   % target hold in binocular rivalry task (name is obviously exchangeable, assign a UNIQUE number to it in the begionning of monkeypsych)
            ...
            for n_obj=1:dyn.n_eye_fix
                par_temp = task.eye.fix(n_obj);
                par_temp.color = task.eye.fix(n_obj).color_dim;
                par_temp.shape.mode = 'circle'; [~,warnID] = lastwarn; warning('off', warnID);
                par_temp.size  = task.eye.fix.size;
                par_temp.shape = task.eye.fix.shape;
                par_eye(n_obj)= aux_FillPar(par_temp);
            end
...
%             dyn.memoryBuffer = cat(1, dyn.memoryBuffer, dyn.pseudo_memoryBuffer);
            ...
            elseif success == 1 && VAR.condition(VAR.current_trial + 1) == 6  % end of a triad (i.e. next state is STATE.success)
                ...
                trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 1];
            elseif success == 0 % allow break of fixation (i.e. go back to STATE.BIN_RIV_ACQ)
                if VAR.reward_system && dyn.time_spent_in_state > 0.5
                    VAR.tRewardStock = VAR.tRewardStock + dyn.time_spent_in_state;
                    VAR.tLastReward = 0;
                end
                success = -1;
                dyn.percept_switch = 1;
...
%                 trial(dyn.trialNumber).success = [trial(dyn.trialNumber).success 0];
            elseif success == 1 && VAR.condition(VAR.current_trial + 1) ~= 5 && VAR.condition(VAR.current_trial + 1) ~= 6
                if VAR.reward_system
                    VAR.tRewardStock = VAR.tRewardStock + (GetSecs - VAR.tLastReward);
                    VAR.tLastReward = 0;
                end
                dyn.time_spent_exploring = 0;
                dyn.time_same_targets_were_presented_HOL = 0;
...
%                 
            ...
            if  strcmp(SETTINGS.background_image, 'binoriv') || strcmp(SETTINGS.background_image, 'BinoRiv_training')
            else
                Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
                displaybackground(SETTINGS,task,dyn)
                Screen(SETTINGS.window,'Flip');
...
%                 trial(dyn.trialNumber).reward_time = NaN;
            ...
            if strcmp(SETTINGS.background_image, 'BinoRiv_training')
            else
                Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
                displaybackground(SETTINGS,task,dyn)
                Screen(SETTINGS.window,'Flip');
...
%                 trial(dyn.trialNumber).reward_time = [trial(dyn.trialNumber).reward_time {'NaN'}];
                ...
                if VAR.report
                    ...
                    trial(dyn.trialNumber).pupil_size = dyn.memoryBuffer(:,15);
                    dyn.memoryBuffer    = single(nan(SETTINGS.bufferSize,15));    % reset memoryBuffer
                    dyn.counterLine     = 0;
                else
...
%                 dyn.state               = state_transition(task,success,dyn.state);
            elseif VAR.Skip_IfFailure == 0 && dyn.state~=STATE.CLOSE && dyn.state~=STATE.BIN_RIV_ACQ
                dyn.state               = state_transition(task,success,dyn.state);
            elseif VAR.Skip_IfFailure == 0 && dyn.state==STATE.BIN_RIV_ACQ
                if VAR.condition(VAR.current_trial+1) == 5
                    dyn.state               = STATE.ITI;
                else
                    dyn.state               = STATE.BIN_RIV_ACQ;
                end
...
if task.type == 15
    global VAR
    if dyn.percept_switch == 0
        if ~VAR.playMovie && dyn.state~=STATE.FIX_ACQ
            Screen(SETTINGS.window,'Flip');
        end
    end
    flip_counter = 0;
...
%     KbQueueCreate(); % Create a keyboard queue for recording button press events
%     KbQueueStart(); % Start recording keyboard events
elseif strcmp(SETTINGS.background_image, 'BinoRiv_training')
    KbQueueCreate(); % Create a keyboard queue for recording button press events
    KbQueueStart(); % Start recording keyboard events
    Screen(SETTINGS.window,'Flip');
else
...
%             end
%             % Check if the Ctrl button is being held down
%             if firstPress(KbName('LeftControl'))
%                 % Ctrl button is being held down, set ctrlButtonPressed to true
%                 repo_blue = 1;
%             else
%                 % Ctrl button is not being held down, set ctrlButtonPressed to false
...
%         end
%         if VAR.condition(VAR.current_trial)==1 % F4 calibrates for fixation cue
%             if firstPress(KbName('F4')) && isfield(par_eye,'deg')% F4 
%                 fprintf(' Updating offsets x_eye = %.2f, y_eye =  %.2f \n', x_eye, y_eye );
%                 task.eye.offset_x = task.eye.offset_x - x_eye;
%                 task.eye.offset_y = task.eye.offset_y - y_eye;
%             end
%         end
    ...
    if strcmp(SETTINGS.background_image, 'binoriv') 
        ...
        if VAR.report
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue,fix_colour, pupil_size];
        else
...
%         if flip_counter == 0 && GetSecs - tEnterState > 5
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 1 && GetSecs - tEnterState > 10
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 2 && GetSecs - tEnterState > 15
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 3 && GetSecs - tEnterState > 20
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 4 && GetSecs - tEnterState > 25
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 5 && GetSecs - tEnterState > 30
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         elseif flip_counter == 6 && GetSecs - tEnterState > 35
%             binoriv_stimuli(VAR.condition(VAR.current_trial), VAR.images)
%             Screen('Flip', SETTINGS.window);
%             flip_counter = flip_counter + 1;
%         end
            ...
            for obj = start_obj_eye:n_obj_eye %obj = dyn.target_selected(1) %obj = 1:n_obj_eye Change 20140519
                if task.type == 15
                    ...
                    if VAR.report
                        ...
                        dyn.memoryBuffer(dyn.counterLine,15) = pupil_size;
                    else
                        if strcmp(fix_colour,'Red')
...
%                             dyn.state   = STATE.ABORT;
    ...
    if task.type == 15
        if VAR.reward_system
            reward_timeLapse = GetSecs - tEnterState + VAR.tRewardStock - tLastReward_withinLoop;
            if reward_timeLapse > VAR.reward_time_storage
                VAR.reward = true;
                VAR.reward_time_storage = task.reward_lowerBound + (task.reward_upperBound - task.reward_lowerBound) * rand;
                VAR.tLastReward = GetSecs;
                tLastReward_withinLoop = GetSecs - tEnterState;
                VAR.tRewardStock = 0;
            end
...
if task.type == 15 || strcmp(SETTINGS.background_image, 'BinoRiv_training')
    Screen(SETTINGS.window,'Flip');
else
    Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
    displaybackground(SETTINGS,task,dyn)
...
if task.type == 15
    global VAR
    if dyn.state == STATE.FIX_HOL || dyn.state == STATE.BIN_RIV_HOL
        tLeaveFixationEye   = 0;
    elseif VAR.binoriv_taskType == 4 && dyn.state == STATE.BIN_RIV_ACQ
    else
        Screen(SETTINGS.window,'Flip');
        flip_timing = 4:4:124;
        flip_counter = 1;
...
while true
    ...
    if task.type == 15
        ...
        if VAR.reward_system
            if VAR.reward
                if dyn.state == STATE.FIX_HOL || dyn.state == STATE.BIN_RIV_HOL % assuming reward is on
                    par.deg.x(1,1) = task.eye.fix.x.red; par.deg.x(2,1) = task.eye.fix.x.blue; 
                    par.deg.y(1,1) = task.eye.fix.y.red; par.deg.y(2,1) = task.eye.fix.y.blue;
                    par.deg.radius = task.eye.fix.radius;
                    par.radiusShape = task.eye.fix.shape;

                    [is_within, fix_colour] = aux_IsWithinRadius_binoriv(x_eye, y_eye, par, dyn);
                    if strcmp(fix_colour,'Red')
                        fix_colour_label = 2;
                    elseif strcmp(fix_colour,'Blue')
                        fix_colour_label = 3;
        ...
        dyn.counter_onset = dyn.counter_onset + 1;
        if VAR.report && VAR.binoriv_taskType ~= 4
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,NaN,NaN,NaN,NaN];
        elseif VAR.report && VAR.binoriv_taskType == 4
            [~, ~, keyCode] = KbCheck();
            if any(find(keyCode)==18)
                % Alt button is being held down, set altButtonPressed to true
                repo_red = 1;
            ...
            while any(find(keyCode)==115); [~, ~, keyCode] = KbCheck; end
            dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,repo_red,repo_blue,fix_colour_label, pupil_size];
        elseif VAR.reward && ~VAR.report
            if dyn.state == STATE.FIX_HOL || dyn.state == STATE.BIN_RIV_HOL
                dyn.memoryBuffer(dyn.counterLine,:) = [tSample-SETTINGS.time_start,dyn.trialNumber,dyn.state,x_hnd,y_hnd,x_eye,y_eye,sen1,sen2,sen3,sen4,fix_colour_label];
            end
        else
    ...
    if task.type == 15
        if dyn.state == STATE.FIX_HOL || dyn.state == STATE.BIN_RIV_HOL
        elseif dyn.state == STATE.BIN_RIV_ACQ
        else
    ...
    if task.type == 15 
        if dyn.state == STATE.FIX_HOL || dyn.state == STATE.BIN_RIV_HOL
            if GetSecs > tEnterState + task.reward.time_neutral    	% check time
                success = true;
                break;
            end
        elseif dyn.state == STATE.BIN_RIV_ACQ
            if GetSecs > tEnterState + task.task4_cue_dur    	% check time
                success = false;
                break;
            end
...
if SETTINGS.useParallel && task.type ~= 15
...
switch shape
    ...
    case 'circle'
        ...
        elseif strcmp(SETTINGS.background_image, 'BinoRiv_training')
            ...
            Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
            if VAR.fix_loc_label == 1
                file = ['Face_and_Body_binocular_FaceFixPosition1_BodyFixPosition3.tif'];
            elseif VAR.fix_loc_label == 2
                file = ['Face_and_Body_binocular_FaceFixPosition2_BodyFixPosition2.tif'];
            elseif VAR.fix_loc_label == 3
                file = ['Face_and_Body_binocular_FaceFixPosition3_BodyFixPosition1.tif'];
            elseif VAR.fix_loc_label == 4
                file = ['Face_and_Body_binocular_FaceFixPosition4_BodyFixPosition4.tif'];
            elseif VAR.fix_loc_label == 5
                file = ['Face_and_Body_binocular_FaceFixPosition3_BodyFixPosition1.tif'];
            elseif VAR.fix_loc_label == 6
                file = ['Face_and_Body_binocular_FaceFixPosition2_BodyFixPosition2.tif'];
            elseif VAR.fix_loc_label == 7
                file = ['Face_and_Body_binocular_FaceFixPosition1_BodyFixPosition3.tif'];
            elseif VAR.fix_loc_label == 8
                file = ['Face_and_Body_binocular_FaceFixPosition4_BodyFixPosition4.tif'];
            end
            
            if VAR.fix_loc_label <= 4 % Red
                for index = 1:length(VAR.images_phys_face)
                    if strcmp(VAR.images_phys_face(index).name, file)
                        image = VAR.images_phys_face(index).data;
                        break
                    end
                end
                binoriv_stimuli(2, image)
                Screen('FillRect', SETTINGS.window, [0 0 0], par.stimRect);
                Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA,[0.5 0 0 1]);
                par.stimRect(1)=par.stimRect(1)+1; par.stimRect(2)=par.stimRect(2)+1; par.stimRect(3)=par.stimRect(3)-1; par.stimRect(4)=par.stimRect(4)-1; 
                Screen('FillOval', SETTINGS.window, [255 0 0], par.stimRect);
            elseif VAR.fix_loc_label >= 5 % Blue
                for index = 1:length(VAR.images_phys_obj)
                    if strcmp(VAR.images_phys_obj(index).name, file)
                        image = VAR.images_phys_obj(index).data;
                        break
                    end
                end
                binoriv_stimuli(3, image)                
                Screen('FillRect', SETTINGS.window, [0 0 0], par.stimRect);
%                 Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA,[0 0 0.5 1]);
                Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA,[0.5 0 0 1]);
                par.stimRect(1)=par.stimRect(1)+1; par.stimRect(2)=par.stimRect(2)+1; par.stimRect(3)=par.stimRect(3)-1; par.stimRect(4)=par.stimRect(4)-1; 
                Screen('FillOval', SETTINGS.window, [255 0 0], par.stimRect);
            end                
...
%                     Screen('FillOval', SETTINGS.window, [0 0 0], [SETTINGS.screen_w_pix/2-VAR.radius_fp SETTINGS.screen_h_pix/2-VAR.radius_fp SETTINGS.screen_w_pix/2+VAR.radius_fp SETTINGS.screen_h_pix/2+VAR.radius_fp]);
                    Screen('FillOval', SETTINGS.window, par.stimColor, par.stimRect);
                    Screen(SETTINGS.window,'Flip');
                elseif VAR.binoriv_taskType==4
                    if VAR.condition(VAR.current_trial)==2 || VAR.condition(VAR.current_trial)==3 || VAR.condition(VAR.current_trial)==4
                        Screen('FillOval', SETTINGS.window, [50 50 50], par.stimRect(:,1)');
                        Screen('FillOval', SETTINGS.window, [50 50 50], par.stimRect(:,2)');
                    end
                end
            else
                Screen('FillOval', SETTINGS.window, par.stimColor, par.stimRect);
...
Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
Screen('FillRect', SETTINGS.window, SETTINGS.BG_COLOR);
% Screen('FillRect', SETTINGS.window, [0 0 0]);


% stim = [0 0 size(stimulus,1)/2]; % (x, y, size)
% % stim(3) = deg2pix_withOffset(stim(3),0);
% [stim(1) stim(2)] = deg2pix_xy(stim(1), stim(2));
% stim_coordinate = aux_pr2rect([stim(1)  stim(2)]', stim(3));
...
% 	Screen('DrawTexture', SETTINGS.window, face,[],stim_coordinate)
    Screen('DrawTexture', SETTINGS.window, face)
    if VAR.binoriv_taskType == 2 || VAR.binoriv_taskType == 3
        Screen('FillOval', SETTINGS.window, [0 0 0], [SETTINGS.screen_w_pix/2-VAR.radius_fp SETTINGS.screen_h_pix/2-VAR.radius_fp SETTINGS.screen_w_pix/2+VAR.radius_fp SETTINGS.screen_h_pix/2+VAR.radius_fp]);
    end
elseif condition == 3
    Screen('BlendFunction', SETTINGS.window, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, [1 1 1 1]);
    obj = Screen('MakeTexture', SETTINGS.window, stimulus);
% 	Screen('DrawTexture', SETTINGS.window, obj,[],stim_coordinate)
...
% 	Screen('DrawTexture', SETTINGS.window, bino,[],stim_coordinate)
...
if strcmp(SETTINGS.background_image, 'binoriv')
    ...
    elseif VAR.binoriv_taskType == 1 || VAR.binoriv_taskType == 2 || VAR.binoriv_taskType == 3
        if VAR.condition(VAR.current_trial) == 4 % binocular rivalry
            switch par.radiusShape
                case 'circle'
    ...
    elseif VAR.binoriv_taskType == 4
        if VAR.condition(VAR.current_trial) == 4 || VAR.condition(VAR.current_trial) == 2 || VAR.condition(VAR.current_trial) == 3
            switch par.radiusShape
                case 'circle'
                    if sqrt(((x0(1,1) - x))^2 + (y0(1,1) - y)^2) < r
                        is_within = 1;
...
function task = FP_loc_shuffle(task)
global VAR

if VAR.binoriv_taskType == 2
    fix_loc_label = [1;1];
...
elseif VAR.binoriv_taskType == 1 || VAR.binoriv_taskType == 3 || VAR.binoriv_taskType == 4
