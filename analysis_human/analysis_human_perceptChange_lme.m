...
lme_switchEffect = fitlme(preprocessedData_switchEffect_table, 'PCT_ins_median ~ Task + (1|SubjectID)');
disp(lme_switchEffect);
randomEffects(lme_switchEffect)

% FPj effect
lme_FPjEffect = fitlme(preprocessedData_FPjEffect_table, 'PCT_ins_median ~ Task + (1|SubjectID)');
disp(lme_FPjEffect);
randomEffects(lme_FPjEffect)

% Saccade effect
lme_SacEffect = fitlme(preprocessedData_SacEffect_table, 'PCT_ins_median ~ Task + (1|SubjectID)');
disp(lme_SacEffect);
randomEffects(lme_SacEffect)

% Saccade+FPj effect
lme_interaction = fitlme(preprocessedData_interaction_table, 'PCT_ins_median ~ WithSaccade*WithFPj + (1|SubjectID)');
